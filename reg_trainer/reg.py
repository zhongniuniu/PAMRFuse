...
# local
from reg_trainer.layers import DownBlock, Conv, ResnetTransformer
...
class ResUnet(torch.nn.Module):
    def __init__(self, nc_a, nc_b, cfg, init_func, init_to_identity):    #ResUnet(self.in_channels_a, self.in_channels_b, cfg='A', init_func=init_func, init_to_identity=init_to_identity).to(self.device)
        super(ResUnet, self).__init__()
        act = down_activation[cfg]   #leaky_relu
        # ------------ Down-sampling path
        self.ndown_blocks = len(ndf[cfg])   #len([32, 64, 64, 64, 64, 64, 64]) 7
        self.nup_blocks = len(nuf[cfg])     #len([[64, 64, 64, 64, 64, 64, 32]]) 7
        assert self.ndown_blocks >= self.nup_blocks
        in_nf = nc_a + nc_b   # 2
        conv_num = 1
        skip_nf = {}
        for out_nf in ndf[cfg]:   #[32, 64, 64, 64, 64, 64, 64]
            setattr(self, 'down_{}'.format(conv_num),DownBlock(in_nf, out_nf, 3, 1, 1, activation=act, init_func=init_func, bias=True,use_resnet=use_down_resblocks[cfg], use_norm=False))
...
class Reg(nn.Module):
    def __init__(self,height,width,in_channels_a,in_channels_b):
        ...
        self.oh, self.ow = height, width    #256,256
        self.in_channels_a = in_channels_a   #1
        self.in_channels_b = in_channels_b   #1
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.offset_map = ResUnet(self.in_channels_a, self.in_channels_b, cfg='A', init_func=init_func, init_to_identity=init_to_identity).to(self.device)  #构建网络
